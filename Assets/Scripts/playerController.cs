...
public class playerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.H))//H en el teclado para usar pociones si el contador de estas es mayor a 0 y solo se podran usar si la vida del jugador es menor a 3
        {
            ...
            {
                playerHealth++;//se le agregara 1 de vida que es representado por el ++ si se toma la pocion, si quisiera usar pociones que curen mas usaria +=2 para curar dos
                PlayerPrefs.SetInt("HealthPotionCount", healthPotionCount - 1);//se bajara el contador de pociones al usarlas
            }
        }
